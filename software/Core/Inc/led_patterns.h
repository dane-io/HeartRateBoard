...
uint8_t (*pulse_step_list[PULSE_STEPS])[][2] = {&pulse_step0, &pulse_step1, &pulse_step2, &pulse_step3, &pulse_step4};
...
uint8_t (*tens_digits[10])[][2] = {&zero_tens, &one_tens, &two_tens, &three_tens, &four_tens, &five_tens, &six_tens, &seven_tens, &eight_tens, &nine_tens};
uint8_t (*ones_digits[10])[][2] = {&zero_ones, &one_ones, &two_ones, &three_ones, &four_ones, &five_ones, &six_ones, &seven_ones, &eight_ones, &nine_ones};
